...
const Toast = ({ type = "success", message, duration = 3000, onClose }) => {
  ...
  useEffect(() => {
    // Trigger visibility after mounting
    setTimeout(() => {
      setVisible(true);
    }, 10); // Add a tiny delay to ensure transition works when appearing

    // Automatically hide the toast after the duration
    const timer = setTimeout(() => {
      setVisible(false);

      // Call onClose after the disappearance animation
      setTimeout(() => {
        if (onClose) onClose();
  ...
  const toastStyles = {
    success: {
      container: "flex items-center w-full px-5 p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800",
  ...
  return (
    ...
    >
      ...
      <div className="ms-3 text-md font-normal">{message}</div>
      <button
        type="button"
