...
import { fetchAddUnit } from "../../../API/unit";
import convertToFormData from "../../../helpers/formdata";

const AddUnit = ({ user, token }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetchAddUnit(body, token);
    ...
    <div className="w-full p-6 bg-white shadow-xl rounded-lg">
      <form onSubmit={handleSubmit}>
          ...
          <div className="w-full md:w-1/3">
            ...
            <div className="space-y-4">
              <div className="w-full aspect-square bg-gray-100 rounded-lg">
                {/* Main image preview */}
                {formData.image && <img src={URL.createObjectURL(formData.image)} alt="Preview" className="w-full h-full object-cover rounded-lg" />}
              </div>
              <input type="file" accept="image/*" name="image" onChange={handleChange} className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-gray-100" required />
            </div>
          </div>
        ...
        <div className="mt-6 flex flex-col sm:flex-row justify-end gap-x-2 gap-y-2 sm:gap-y-0">
          <button type="submit" className="px-6 py-2 rounded text-white bg-blue-600 hover:bg-blue-500 text-nowrap" disabled={isLoading}>
            {isLoading ? "Menunggu..." : "Tambah Unit"}
          </button>
